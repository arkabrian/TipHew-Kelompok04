...
void addTxtToLinkedListAnjing(struct kandangAnjing **head){
	if(access("kandangAnjing.txt", F_OK) == 0){ //Ngecek apakah file ada atau engga
		...
		while(fgets(temp, sizeof(temp), kandang)){
			...
			token = strtok(NULL, ","); //sisa hari
			dog.sisaHari = atoi(token); //sisa hari
			...
			token = strtok(NULL, ","); //nitipin berapa hari
			dog.nitipHari = atoi(token); //nitipin berapa hari
...
void addTxtToLinkedListKucing(struct kandangKucing **head){
	if(access("kandangKucing.txt", F_OK) == 0){ //Ngecek apakah file ada atau engga
		...
		while(fgets(temp, sizeof(temp), kandang)){
			...
			token = strtok(NULL, ","); //sisa hari
			cat.sisaHari = atoi(token); //sisa hari
			...
			token = strtok(NULL, ","); //nitipin berapa hari
			cat.nitipHari = atoi(token); //nitipin berapa hari
...
void nextDayAnjing(struct kandangAnjing **head){
	if(access("kandangAnjing.txt", F_OK) == 0){
		...
		while(fgets(temp, sizeof(temp), kandang)){
			...
			token = strtok(NULL, ","); //sisa hari
			dog.sisaHari = atoi(token) - 1; //sisa hari akan decrement 
			...
			fprintf(tempKandang, "%s,%d,%s,%s,%d,\n", dog.pemilik, dog.sisaHari, dog.nama, dog.jenis, dog.nitipHari); 
		}
		fclose(kandang);
		...
		while(tempL != NULL){
			(tempL->data.sisaHari)--; //sisa hari akan decrement 
			tempL = tempL->next;
		}
...
void nextDayKucing(struct kandangKucing **head){
	if(access("kandangKucing.txt", F_OK) == 0){
		...
		while(fgets(temp, sizeof(temp), kandang)){
			...
			token = strtok(NULL, ","); //sisa hari
			cat.sisaHari = atoi(token) - 1; //sisa hari akan decrement 
			...
			fprintf(tempKandang, "%s,%d,%s,%s,%d,\n", cat.pemilik, cat.sisaHari, cat.nama, cat.jenis, cat.nitipHari);
		}
		fclose(kandang);
...
void extendAnjing(struct kandangAnjing **head, char username[], int hari){
	...
	while(fgets(temp, sizeof(temp), kandang)){
		...
		if(strcmp(token, username) == 0){
			...
			token = strtok(NULL, ","); //sisa hari
			dog.sisaHari = atoi(token) + hari; //sisa hari akan ditambah
			...
			token = strtok(NULL, ","); //nitipin berapa hari
			dog.nitipHari = atoi(token) + hari; //nitipin berapa hari akan ditambah
	...
	while(tempL != NULL){
		if(strcmp(tempL->data.pemilik, username) == 0)
			(tempL->data.sisaHari) += hari; //sisa hari akan ditambah
			(tempL->data.nitipHari) += hari; //nitipin berapa hari akan ditambah
		tempL = tempL->next;
	}
}
void extendKucing(struct kandangKucing **head, char username[], int hari){
	...
	while(fgets(temp, sizeof(temp), kandang)){
		...
		if(strcmp(token, username) == 0){
			...
			token = strtok(NULL, ","); //sisa hari
			cat.sisaHari = atoi(token) + hari; //sisa hari akan ditambah
			...
			token = strtok(NULL, ","); //nitipin berapa hari
			cat.nitipHari = atoi(token) + hari; //nitipin berapa hari akan ditambah
...
void bubbleSortAscAnjing(struct kandangAnjing *head){
	...
	do{
		...
		while(ptr1->next != lptr){
			if(ptr1->data.sisaHari > ptr1->next->data.sisaHari){
...
void tambahHewanKeKandang(){
	...
	if(kucingAnjing == 1){
		...
		scanf("%d", &cat.nitipHari);
		cat.sisaHari = cat.nitipHari;
		printf("\nNama Kucing: ");
		scanf(" %[^\n]s", &cat.nama);
		printf("\nJenis Kucing: ");
		scanf(" %[^\n]s", &cat.jenis);
		...
		if(fprintf(kandangKucing, "%s,%d,%s,%s,%d,\n", cat.pemilik, cat.sisaHari, cat.nama, cat.jenis, cat.nitipHari)){
	...
	//Anjing
		...
		scanf(" %[^\n]s", &dog.pemilik);
		printf("\nMasukkan lama penitipan (hari): ");
		scanf("%d", &dog.nitipHari);
		dog.sisaHari = dog.nitipHari;
		printf("\nNama Anjing: ");
		scanf(" %[^\n]s", &dog.nama);
		printf("\nJenis Anjing: ");
		scanf(" %[^\n]s", &dog.jenis);
		...
		if(fprintf(kandangAnjing, "%s,%d,%s,%s,%d,\n", dog.pemilik, dog.sisaHari, dog.nama, dog.jenis, dog.nitipHari)){
...
void pelepasanHewan(struct kandangKucing **headCat, struct kandangAnjing **headDog){
	...
	int i = 1;
	int sisaHari;
	int nitipHariAnjing = 0, nitipHariKucing = 0;
	int found = 0;
	int anjing = 0, kucing = 0;
	char nama[100];
	char temp[100];
	char temp2[100];
	char *token;
	...
	printf("================Kucing===============\n");
	while(tempCat != NULL){
		if(tempCat->data.sisaHari <= 0){
			printf("%d.\n", i);
			printf("Pemilik        : %s\n", tempCat->data.pemilik);
			printf("Nama           : %s\n", tempCat->data.nama);
			printf("Jenis          : %s\n", tempCat->data.jenis);
			printf("Lama Penitipan : %d\n", tempCat->data.nitipHari);
			printf("Sisa hari      : %d\n", tempCat->data.sisaHari);
			i++;
			found++;
			kucing++;
		}
	...
	printf("================Anjing===============\n");
	while(tempDog != NULL){
		if(tempDog->data.sisaHari <= 0){
			printf("%d.\n", i);
			printf("Pemilik        : %s\n", tempDog->data.pemilik);
			printf("Nama           : %s\n", tempDog->data.nama);
			printf("Jenis          : %s\n", tempDog->data.jenis);
			printf("Lama Penitipan : %d\n", tempDog->data.nitipHari);
			printf("Sisa hari      : %d\n", tempDog->data.sisaHari);
			i++;
			found++;
			anjing++;
		}
	...
	printf("Jumlah Kucing yang sisa harinya habis %d\n", kucing);
	printf("Jumlah Anjing yang sisa harinya habis %d\n", anjing);
	if(found > 0){
		...
		while(fgets(temp, sizeof(temp), kandangAnjing)){
			...
			if(strcmp(token, nama) == 0){
				...
				if(sisaHari <= 0){
					...
					token = strtok(NULL, ",");//jenis
					token = strtok(NULL, ",");//nitipin berapa hari
					nitipHariAnjing += atoi(token) * 75000;
					//continue;
				}
		...
		while(fgets(temp, sizeof(temp), kandangKucing)){
			...
			if(strcmp(token, nama) == 0){
				...
				if(sisaHari <= 0){
					...
					token = strtok(NULL, ",");//jenis
					token = strtok(NULL, ",");//nitipin berapa hari
					nitipHariKucing += atoi(token) * 50000;
					//continue;
				}
		...
		printf("Total anda		: %d\n", nitipHariKucing + nitipHariAnjing);
		system("pause");
